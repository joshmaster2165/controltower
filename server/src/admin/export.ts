...
export interface PathRow {
  ...
  inspected_by: string[];
  key_permits: boolean;
  /** Agents these calls were made for (each chain's origin), when the agent was acting for others. */
  for_agents: string[];
}
...
export interface DataflowInventory {
  ...
  agents: Array<{ id: string; name: string; team: string | null; project: string | null; zones: string[]; models_allowed: string[]; tools_allowed: string[]; requests: number; spend_nanousd: number; spent_for_it_nanousd: number; last_used: number | null }>;
...
export async function buildInventory(ctx: AppContext, hours: number): Promise<DataflowInventory> {
  ...
  for (const r of rows) {
    ...
    paths.push({
      ...
      key_permits: permits,
      for_agents: r.origins ? String(r.origins).split(',').filter(Boolean).sort() : [],
    });
  ...
  const agents = [...ctx.registry.keysById.values()]
    ...
    .map((k) => ({
      ...
      // Spend other agents made on this agent's behalf, anywhere down its chains (by agent ID).
      spent_for_it_nanousd: spentFor.get(k.agentId ?? k.id) ?? 0,
...
export function inventoryCsv(inv: DataflowInventory): string {
  const cols: Array<keyof PathRow> = ['agent', 'team', 'kind', 'target', 'provider', 'tool', 'operation', 'requests', 'errors', 'blocked', 'held', 'spend_nanousd', 'tokens', 'access', 'access_gate', 'inspected_by', 'key_permits', 'for_agents', 'last_seen'];
...
export function inventoryMarkdown(inv: DataflowInventory): string {
  ...
  out.push('| Agent | Target | Via | Requests | Errors | Blocked | Held | Spend | Access | Inspected by | Key allows | For |', '|---|---|---|---:|---:|---:|---:|---:|---|---|---|---|');
  for (const p of inv.paths) {
    const target = p.kind === 'tool' ? `${p.target} → ${p.tool} (${p.operation})` : p.target;
    out.push(`| ${mdCell(p.agent)} | ${mdCell(target)} | ${mdCell(p.provider)} | ${n(p.requests)} | ${n(p.errors)} | ${n(p.blocked)} | ${n(p.held)} | ${usd(p.spend_nanousd)} | ${ACCESS[p.access]}${p.access_gate ? ` (${mdCell(p.access_gate)})` : ''} | ${mdCell(p.inspected_by.join(', ') || '—')} | ${p.key_permits ? 'yes' : '**no**'} | ${mdCell(p.for_agents.join(', ') || '—')} |`);
  }
  out.push('', '## Agents', '', 'Spend is the agent\'s own; *spent for it* is what other agents spent acting on its behalf.', '', '| Agent | Team | Project | Zones | Models allowed | Tools allowed | Requests | Spend | Spent for it |', '|---|---|---|---|---|---|---:|---:|---:|');
  for (const a of inv.agents) out.push(`| ${mdCell(a.name)} | ${mdCell(a.team ?? '—')} | ${mdCell(a.project ?? '—')} | ${mdCell(a.zones.join(', ') || '—')} | ${mdCell(a.models_allowed.join(', '))} | ${mdCell(a.tools_allowed.join(', '))} | ${n(a.requests)} | ${usd(a.spend_nanousd)} | ${a.spent_for_it_nanousd ? usd(a.spent_for_it_nanousd) : '—'} |`);
