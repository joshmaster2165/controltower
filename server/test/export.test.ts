...
const inv: DataflowInventory = {
  ...
  observed: [{ agent: 'researcher', team: null, target: 'api.openai.com', system: 'OpenAI', kind: 'model', bypass: true, calls: 4, errors: 0, writes: 4, last_seen: 0 }],
};
...
describe('data-flow inventory export', () => {
  ...
  it('writes Markdown tables that survive pipes, and flags paths the key does not allow', () => {
    ...
    expect(md).toContain('| researcher | OpenAI (api.openai.com) | model | 4 | 4 | 0 | **calls a model provider directly — bypasses the gateway** |');
  });
});
