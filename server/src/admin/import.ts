...
import { ImportError, planConfigImport, publicPlan, type ImportPlan } from '../importers/config.js';

const MAX_YAML = 1_000_000;

/** Import a config.yaml once from the console: preview, then apply. */
export async function importRoutes(app: FastifyInstance, ctx: AppContext): Promise<void> {
  ...
  const plan = (yaml: unknown): ImportPlan => {
    if (typeof yaml !== 'string' || !yaml.trim()) throw new ImportError('Paste the contents of your config.yaml.');
    if (yaml.length > MAX_YAML) throw new ImportError('That file is larger than 1 MB.');
    return planConfigImport(yaml, process.env, {
  ...
  app.post('/admin/api/import/config/plan', { preHandler: guard, bodyLimit: MAX_YAML + 4096 }, async (req, reply) => {
    try {
  ...
  app.post('/admin/api/import/config/apply', { preHandler: guard, bodyLimit: MAX_YAML + 64 * 1024 }, async (req, reply) => {
    ...
    ctx.log.info({ providers: result.providers, deployments: result.deployments, aliases: result.aliases, mcp: result.mcp_servers }, 'imported config file');
    if (p.settings.masterKey) result.warnings.push('general_settings.master_key applies when this file is loaded with --config (or set CT_ADMIN_KEY).');
    return result;
  });
}
